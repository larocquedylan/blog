...
`src/graphql/mutations/register`, which gets the `RegularUser` fragment from `src/graphql/fragments/RegularUser`
...
The user will then be brought to the index page, which is a list of the posts fetched from the PostgreSQL database.
